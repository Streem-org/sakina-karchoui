...
class HelpView(discord.ui.View):
    ...
    @discord.ui.select(
        ...
        options=[
            ...
            discord.SelectOption(label="Misc", emoji="🏷️")
        ]
    )
...
""",
            ...
            embed = discord.Embed(
                title="🏷️ Misc Cogs",
                description="""
`.convert`- Convert economy
`.urban` - View the Urban Dictionary
`.imgify` - Imgify a link
""",
                color=0x2b2d31
            )            
...
async def urban(ctx, *, term: str):
    ...
    try:
        ...
        embed.add_field(name="👍 Upvotes", value=data["thumbs_up"], inline=True)
        embed.add_field(name="👎 Downvotes", value=data["thumbs_down"], inline=True)

        embed.set_footer(text="Source: Urban Dictionary")
...
# ---------------- IMGIFY ---------------- #
@bot.hybrid_command(name="imgify")
async def imgify(ctx, url: str = None):
    ...
    if url and url.startswith("http"):
        ...
        if replied:
            ...
            elif replied.content.startswith("http"):
                image_url = replied.content

    if not image_url:
        return await ctx.send("❌ Provide an image or URL.")

    embed = discord.Embed(title="🖼️ Image Viewer", color=0x2b2d31)
